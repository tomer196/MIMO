...
class ResNet(nn.Module):
    def __init__(self, in_chans, out_chans, chans, num_blocks, drop_prob):
        ...
        self.conv = ComplexConv2d(in_chans, in_chans, kernel_size=3, padding=1)
        self.bn = ComplexBatchNorm2d(in_chans)
        self.relu = nn.ReLU(inplace=True)
        block = ResidualBlock
        ...
        self.conv_out = ComplexConv2d(20, 20, kernel_size=3, padding=1)

    def make_layer(self, block, out_channels, blocks, stride=1):
        downsample = None
        if (stride != 1) or (self.in_channels != out_channels):
            downsample = nn.Sequential(
                ComplexConv2d(self.in_channels, out_channels, stride=stride, kernel_size=3, padding=1),
    ...
    def forward(self, x):
        x = x.reshape(x.shape[0], 20, -1, x.shape[-1]).transpose(1, 2)
        ...
        out = torch.view_as_complex(out.reshape(x.shape[0], -1, x.shape[-2], 2))
        return out
...
class ComplexCNNModel(nn.Module):
    def __init__(self, in_chans, out_chans, chans, num_blocks, drop_prob):
        ...
        layers = [ComplexInstanceNorm2d(in_chans),
                  ComplexConvBlock(in_chans, out_chans, drop_prob)]
        for i in range(num_blocks - 1):
            layers.append(ComplexConvBlock(out_chans, out_chans, drop_prob))
        layers.append(ComplexConv2d(out_chans, out_chans, kernel_size=3, padding=1))
    ...
    def forward(self, input):
        ...
        Returns:
            (torch.Tensor): Output tensor of shape [batch_size, self.out_chans, height, width]
        """
        input = input.reshape(input.shape[0], 20, -1, input.shape[-1]).transpose(1, 2)
        input = torch.view_as_real(input)
        output = self.net(input)
        output = torch.view_as_complex(output.reshape(input.shape[0], -1, input.shape[-2], 2))
        return output
...
class ComplexAvgPool2d(nn.Module):
    def __init__(self, kernel_size):
        super().__init__()
        self.ap_re = nn.AvgPool2d(kernel_size)
        self.ap_im = nn.AvgPool2d(kernel_size)
