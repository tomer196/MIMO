...
import matplotlib
matplotlib.use('Agg')
from continuous_layer import ContinuousUnetModel
...
def evaluate(args, epoch, model, data_loader, writer, steering_dict):
    ...
    with torch.no_grad():
        if epoch != 0:
            for iter, data in enumerate(data_loader):
                smat_target, elevation = data
                smat_target = smat_target.to(args.device_)
                AzRange_target = beamforming(smat_target, steering_dict, args, elevation)
                AzRange_target = abs(AzRange_target)
                AzRange_target, mean, std = normalize_instance(AzRange_target)
                ...
                psnr_corr_list.append(psnr(AzRange_target, AzRange_corr))
            ...
            writer.add_scalar('PSNR_corr', np.mean(psnr_corr_list), epoch)
...
def visualize(args, epoch, model, data_loader, writer, steering_dict):
    ...
    with torch.no_grad():
        for iter, data in enumerate(data_loader):
            ...
            AzRange_rec = model(smat_target, steering_dict, args, elevation, mean, std)
            AzRange_corrupted = model.sub_sample(smat_target, steering_dict, args, elevation)
...
def build_model(args):
    model = ContinuousUnetModel(args).to(args.device)
    return model
