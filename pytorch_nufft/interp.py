...
def interpolate(input, width, kernel, coord, device):
    ...
    batch_size = int(util.prod(batch_shape))
    ...
    if ndim == 1:
        output = _interpolate1(output, input, width, kernel, coord)
    elif ndim == 2:
...
def _interpolate1(output, input, width, kernel, coord):
    ...
    x0 = torch.ceil(kx - width / 2)

    for x in range(int(width) + 1):

        w = lin_interpolate(kernel, torch.abs(x0 + x - kx) / (width / 2))

        xx = torch.fmod(x0 + x, nx).long()
        output = output + w * input[:, xx]
...
def gridding(input, shape, width, kernel, coord,device):
    ...
    if ndim == 1:
        output = _gridding1(output, input, width, kernel, coord)
    elif ndim == 2:
